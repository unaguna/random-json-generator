...
class TestStrFactory(unittest.TestCase):
    ...
    def test_gen_with_empty_schema(self):
        ...
        ``StrFactory().gen(schema)`` returns a string value. When ``schema`` is empty, the result contains only
        alphabets.

        assert that:
            When the schema is empty, ``StrFactory().gen(schema)`` returns ``str`` value contains only alphabets.
        """
        schema = {}
        generated = StrFactory().gen(schema)
    ...
    def test_gen_with_maxLength_0(self):
        ...
        When ``schema.maxLength`` is specified, ``StrFactory().gen(schema)`` returns a string value with a length of
        ``maxLength`` or less.

        assert that:
            When ``schema.maxLength == 0``, ``StrFactory().gen(schema)`` returns the empty string.
        """
        schema = {"maxLength": 0}
        generated = StrFactory().gen(schema)
    ...
    def test_gen_with_maxLength(self):
        ...
        assert that:
            When ``schema.maxLength``, ``StrFactory().gen(schema)`` returns a string with a length of ``maxLength`` or
            less.
        """
    ...
    def test_gen_with_negative_maxLength(self):
        ...
        ``schema.maxLength`` must be non-negative. When ``schema.maxLength < 0``, ``StrFactory().gen(schema)`` raises
        InvalidSchemaError.

        assert that:
            When ``schema.maxLength < 0``, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "maxLength": -1
        }
        self.assertRaises(InvalidSchemaError, lambda: StrFactory().gen(schema))
    ...
    def test_gen_with_non_integer_maxLength(self):
        ...
        ``schema.maxLength`` cannot divided by 1, ``StrFactory().gen(schema)`` raises InvalidSchemaError.

        assert that:
            When ``schema.maxLength`` cannot divided by 1, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "maxLength": 1.1
        }
    ...
    def test_gen_with_non_number_maxLength(self):
        ...
        ``schema.maxLength`` must be number. When ``schema.maxLength`` isn't number, ``StrFactory().gen(schema)``
        raises InvalidSchemaError.

        assert that:
            When ``schema.maxLength`` isn't number, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "maxLength": "1"
        }
    ...
    def test_gen_with_minLength(self):
        ...
        When ``schema.minLength`` is specified, ``StrFactory().gen(schema)`` returns a string value with a length of
        ``minLength`` or more.

        assert that:
            When ``schema.minLength`` is specified, ``StrFactory().gen(schema)`` returns the string value and it's
    ...
    def test_gen_with_negative_minLength(self):
        ...
        ``schema.minLength`` must be non-negative. When ``schema.minLength < 0``, ``StrFactory().gen(schema)`` raises
        InvalidSchemaError.

        assert that:
            When ``schema.minLength < 0``, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "minLength": -1
        }
    ...
    def test_gen_with_non_integer_minLength(self):
        ...
        ``schema.minLength`` cannot divided by 1, ``StrFactory().gen(schema)`` raises InvalidSchemaError.

        assert that:
            When ``schema.minLength`` cannot divided by 1, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "minLength": 1.1
        }
    ...
    def test_gen_with_non_number_minLength(self):
        ...
        ``schema.minLength`` must be number. When ``schema.minLength`` isn't number, ``StrFactory().gen(schema)``
        raises InvalidSchemaError.

        assert that:
            When ``schema.minLength`` isn't number, ``StrFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "minLength": "1"
        }
    ...
    def test_gen_with_length(self):
        ...
        When ``schema.minLength`` and ``schema.maxLength`` is specified, ``StrFactory().gen(schema)`` returns a string
        ...
        assert that:
            When ``schema.minLength`` and ``schema.maxLength`` is specified, ``StrFactory().gen(schema)`` returns the
    ...
    def test_gen_with_conflicting_length(self):
        ...
        ``minLength > maxLength``, ``StrFactory().gen(schema)`` raises SchemaConflictError.

        assert that:
            When ``schema.minLength > schema.maxLength``, ``StrFactory().gen(schema)`` raises SchemaConflictError.
        """
        thresholds_list = ((0, 1),
                           (12, 15))
        ...
        for max_length, min_length in thresholds_list:
            with self.subTest(min_length=min_length, max_length=max_length):
                schema = {"minLength": min_length, "maxLength": max_length}
                self.assertRaises(SchemaConflictError, lambda: StrFactory().gen(schema))
    ...
    def test_gen_with_pattern(self):
        ...
        assert that:
            When ``schema.pattern`` is valid as regular expression, ``StrFactory().gen(schema)`` returns a string
    ...
    def test_gen_with_pattern_and_minLength(self):
        ...
        assert that:
            ...
            ``StrFactory().gen(schema)`` returns a string satisfies this regular expression.
        """
        pattern_list = ("\\d\\d\\d-\\d\\d\\d\\d-\\d\\d\\d",
                        "[a-z][A-Z]\\d\\d")
    ...
    def test_gen_with_illegal_pattern(self):
        ...
        when ``schema.pattern`` is invalid as regular expression, ``StrFactory().gen(schema)`` raises
        InvalidSchemaError.
...
class TestOptionDefaultLength(unittest.TestCase):
    ...
    def test_default_length_with_schema_min_max(self):
        ...
        for options in options_list:
            generated = StrFactory().gen(schema, options=options)
        ...
        for options in options_list:
            with self.subTest(default_min_length_of_string=options.default_min_length_of_string):
                with self.assertRaises(SchemaConflictError):
                    StrFactory().gen(schema, options=options)
