...
class TestListFactory(unittest.TestCase):
    ...
    def test_gen_with_empty_schema(self):
        ...
        ``ListFactory().gen(schema)`` returns a list even if ``schema`` is empty.

        assert that:
            When the schema is empty, ``ListFactory().gen(schema)`` returns ``list`` value.
        """
        schema = {}
        generated = ListFactory().gen(schema)
    ...
    def test_gen_with_minItems(self):
        ...
        ``ListFactory().gen(schema)`` returns a list. When ``schema.minItems`` is specified, the result list has at
    ...
    def test_gen_with_maxItems(self):
        ...
        ``ListFactory().gen(schema)`` returns a list. When ``schema.maxItems`` is specified, the result list has at
    ...
    def test_gen_with_negative_minItems(self):
        ...
        ``schema.minItems`` must be non-negative. When ``schema.minItems < 0``, ``ListFactory().gen(schema)`` raises
        InvalidSchemaError.

        assert that:
            When ``schema.minItems < 0``, ``ListFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "minItems": -1
        }
        self.assertRaises(InvalidSchemaError, lambda: ListFactory().gen(schema))

    def test_gen_with_negative_maxItems(self):
        """ Semi-normalized System Test

        ``schema.maxItems`` must be non-negative. When ``schema.maxItems < 0``, ``ListFactory().gen(schema)`` raises
        InvalidSchemaError.

        assert that:
            When ``schema.maxItems < 0``, ``ListFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "maxItems": -1
        }
    ...
    def test_gen_with_non_integer_minItems(self):
        ...
        ``schema.minItems`` cannot divided by 1, ``ListFactory().gen(schema)`` raises InvalidSchemaError.

        assert that:
            When ``schema.minItems`` cannot divided by 1, ``ListFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "minItems": 1.1
        }
    ...
    def test_gen_with_non_integer_maxItems(self):
        ...
        ``schema.maxItems`` cannot divided by 1, ``ListFactory().gen(schema)`` raises InvalidSchemaError.

        assert that:
            When ``schema.maxItems`` cannot divided by 1, ``ListFactory().gen(schema)`` raises InvalidSchemaError.
        """
        schema = {
            "maxItems": 1.1
        }
    ...
    def test_gen_with_tight_length(self):
        ...
        When ``schema.minItems`` and ``schema.maxItems`` specified, ``ListFactory().gen(schema)`` returns a list of
    ...
    def test_gen_with_param_conflict_min_max(self):
        ...
        assert that:
            When the schema has ``properties.minItems > properties.maxItems``, ``ListFactory().gen(schema)`` raised
            SchemaConflictError.
        """
        ...
        for max_items, min_items in thresholds_list:
            with self.subTest(min_items=min_items, max_items=max_items):
                ...
                self.assertRaises(SchemaConflictError, lambda: ListFactory().gen(schema))
    ...
    def test_gen_with_tuple_items_and_too_great_minItems_and_additional_false(self):
        ...
        ``len(schema.items) < schema.minItems``, ``ListFactory().gen(schema)`` raises SchemaConflictError.

        assert that:
            When ``schema.additionalItems`` is ``false`` and ``len(schema.items) < schema.minItems``,
            ``ListFactory().gen(schema)`` raises SchemaConflictError.
        """
        schema = {
            "type": "array",
    ...
    def test_gen_with_no_items_and_options_default_schema_of_items(self):
        ...
        generated = ListFactory().gen(schema, options=Options(default_schema_of_items=default_schema))
...
class TestGenlistLengthRange(unittest.TestCase):
    ...
    def test_list_length_range_with_tuple_schema_with_too_large_min_additional_false(self):
        """ Semi-normalized System Test
         ...
         ``ListFactory().gen(schema)`` cannot has elements more than ``len(schema.items)``. So if ``schema.minItem``
